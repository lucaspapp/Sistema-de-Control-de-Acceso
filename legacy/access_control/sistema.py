...
def procesar_entrada(info):
    ...
    # Un evento nuevo puede no tener snapshot lista todavía. Esperamos antes
    # de abandonar el tracking, para no perder rostros de personas prohibidas.
    imagen = None
    for intento in range(12):
        imagen = obtener_snapshot(info["event_id"], info.get("camera"))
        if imagen is not None:
            break
        time.sleep(0.5)
    if imagen is None:
        print(f"[FACE] Sin captura disponible para {tid}; se reintentará con el próximo evento.")
    ...
    for muestra in range(6):
        if muestra > 0:
            time.sleep(0.72)
            nueva_imagen = obtener_snapshot(info["event_id"], info.get("camera"))
    ...
    if prohibido:
        ...
        try:
            ...
            record_alert(nombre, info.get("camera", "unknown"), score, path)
        except Exception as e:
